...
void DrawCells()
{
	...
	if (s_drawui)
	{
		SetRenderScale({ 4.0f, 4.0f }); // Mahollistaa pienemmän textin piirtämisen

		static bool alert_space = true;
		if (alert_space)
		{
			DrawText("Paina Spaceiä pohjassa", s_cellfont, { FIELD_W, FIELD_H - 8}, RED, true);
			DrawText("edistääksesi aikaa"    , s_cellfont, { FIELD_W, FIELD_H + 8 }, RED, true);
			if (IsKeyDown(SDL_SCANCODE_SPACE))
			{
				alert_space = false;
			}
		...
		SetRenderScale({ 8.0f, 8.0f }); // Pistä "zoomi" takas normaaliin
	}
}
